...
| **sherpa-onnx** | Bundled native lib, ONNX Runtime | Loaded on demand | Diarization (TitaNet or 3D-Speaker embeddings + spectral clustering). |
...
- sherpa-onnx is the diarization pipeline. We do not own enough audio /
  ML engineering capacity to reimplement TitaNet embeddings + spectral
  clustering. It's a hard external dependency at the runtime layer.
...
Models are not in the bundle. The installer is ~50-100 MB; the first-run
