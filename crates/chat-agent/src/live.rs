...
//! recording start (~40 s for a moderately sized slide deck) and that prefix KV
//! is reused across every digest refresh. A fresh-per-turn approach would
//! re-pay the ~40 s prefill cost on every cadence tick, making live operation
//! unusable.
//!
//! The prefix is the **open user turn** of a chat-template prompt
//! (`<bos><start_of_turn>user\n{system + categories + attachments}`); the driver
//! supplies that text and the per-refresh tail already wrapped in template
//! markers (this crate only does the KV mechanics, never templating). Each
//! refresh **prunes the KV back to the prefix** and decodes a fresh, BOUNDED
//! tail on top — the running digest + a recent transcript window + the template
//! suffix that closes the user turn and opens the model turn. The held context
//! therefore never grows beyond `prefix + tail + generation`: it cannot
//! overflow on a long meeting (the failure mode the cumulative-append design
//! hit on its first live test — see planning issue 0022). The running digest,
//! re-fed each refresh, is the durable memory; old verbatim transcript is
//! dropped.
//!
//! Caveat (Gemma SWA): the shipped model uses interleaved sliding-window
//! attention, so on the local-attention layers a far-back prefix token stops
//! being attendable once generation runs many positions past it — only the
//! global-attention layers keep full prefix visibility. The prefix KV is still
//! computed once and reused (the cost saving is real and correct); but
//! attachment grounding on Gemma is bounded by its SWA window on the local
//! layers, not the whole `n_ctx`.
//!
//! # Design
//!
...
pub struct LlamaLiveBackend<'m> {
    ...
    /// system + category instructions + attachment markdown). Recorded by
    /// `prefill_prefix` and never re-decoded. Every `refresh` prunes the KV
    /// cache back to exactly this length before appending its tail, so the
    /// expensive attachment prefill is paid once and the held context never
    /// grows beyond `prefix_len + tail + generation`.
    prefix_len: i32,
}
...
impl<'m> LiveSessionBackend for LlamaLiveBackend<'m> {
    fn prefill_prefix(&mut self, prefix_text: &str, cancel: &CancelFlag) -> Result<usize, Error> {
        ...
        // half is always available for the per-refresh tail (running digest +
        // recent transcript window) plus generation. The first live test pinned
        // a 28 730-token attachment prefix into a 32 768 window, leaving ~4 k of
        // headroom and exhausting the context within a few refreshes. The
        // instruction text sits at the FRONT of the prefix (system prompt +
        // category list + JSON contract), so truncating the token tail drops
        // attachment content first and preserves the instructions. A truncated
        // attachment set is a soft degradation; starving the window is not.
        ...
        if tokens.len() > max_prefix_tokens {
            tracing::warn!(
                ...
                "live prefix exceeds half of n_ctx; truncating pinned attachment \
                 context to preserve per-refresh window headroom"
            );
    ...
    fn refresh(
        ...
        // system + category instructions + attachment markdown). Each refresh
        // re-decodes a fresh, BOUNDED tail (the running digest + the recent
        ...
        // tail AND its generated answer in one operation, so `n_past` never
        // grows beyond `prefix_len + tail + generation`. The expensive ~40 s
        // attachment prefill is therefore paid exactly once, at seed time.
