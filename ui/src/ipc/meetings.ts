...
/**
 * Re-run speaker diarization for a meeting (Phase 6, FR-11).
 *
 * The backend command (`rediarize_meeting`) is now on the generated `commands`
 * surface (the Phase-6 diarizer/orchestrator/ipc-bridge JOIN regenerated
 * `bindings.ts`), so this routes through `commands.rediarizeMeeting` like every
 * other command — the temporary shim-aware `callPendingCommand` raw-`invoke`
 * path was collapsed (A9).
 *
 * Tests mock THIS module (per `architecture/cross-cutting.md` — Automated
 * testing policy); they do not fake the generated bindings file.
 *
 * The backend decodes the meeting's pause-INCLUDING PCM
 * (`persistence::reader::read_audio_pcm`), runs the `SherpaDiarizer` over the
 * stored transcript segments (the orchestrator resolves both model dirs via
 * `model-registry`), rewrites `transcript.json`
 * (`persistence::write_transcript`) with the overlaid `speaker_id`s, updates
 * `metadata.json`'s `speaker_count`, refreshes the index row, and emits
 * `AppEvent::DiarizationComplete { meeting_id, speaker_count }` on the shared
 * bus. Long-running (sherpa inference on a `spawn_blocking` task); resolves once
 * the run completes. Like `re_transcribe`, it refuses unless the recorder is
 * `Idle`.
 */
export async function rediarize(meetingId: MeetingId): Promise<void> {
  unwrap(await commands.rediarizeMeeting(meetingId));
}
