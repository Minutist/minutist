 ...
 * `delete_meeting`, `re_transcribe`, `re_summarise`) are present on the
 * generated `commands` surface (Stream C / the Phase-4 backend JOIN regenerated
 * `bindings.ts`). This module reaches them through the shim-aware `commands`
 * from `./client` (NOT raw `./bindings`) — the same single injection point
 * `notes.ts` uses, so the DEV shim and the Vitest mocks both intercept here.
 *
