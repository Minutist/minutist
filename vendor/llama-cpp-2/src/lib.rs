...
// `model.rs`'s own deprecated methods reference the deprecated `Special` enum internally;
// patching this crate in as a path dependency drops cargo's implicit `--cap-lints allow`,
// which would otherwise let those upstream self-uses through silently (see
// architecture/cross-cutting.md — "llama.cpp build + version policy").
#![allow(deprecated)]
