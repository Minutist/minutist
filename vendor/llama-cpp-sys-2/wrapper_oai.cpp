...
// `common_chat_msg_diff_to_json_oaicompat` moved from `common/chat.cpp` into
// `tools/server/` upstream (not vendored by llama-cpp-sys-2's package) —
// inlined here verbatim from its pre-move implementation, since
// `common_chat_msg_diff`'s field shape is unchanged.
//
// The pre-removal wrapper also had `ends_with`/`detect_thinking_forced_open`
// helpers built around the old `common_chat_params::thinking_end_tag`
// (singular `std::string`) field; dropped here rather than adapted to the
// current `thinking_end_tags` (`std::vector<std::string>`) shape, because
// `detect_thinking_forced_open` had no call site in the pre-removal wrapper
// and the concept it computed is now derived inside `chat.cpp` itself from
// `common_chat_parser_params::generation_prompt` (set below at the two
// `syntax.generation_prompt = generation_prompt;` sites) — nothing in this
// file needs to reconstruct it.
